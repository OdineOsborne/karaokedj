# Mixfonia — Mix · Sing · Together

Mixfonia (già KaraokeDJ) è un'app Windows (WPF / .NET 8) per serate di intrattenimento e karaoke: due deck con cambio
...
| **Licenza donationware** | Pulsante **❤ Sostieni Mixfonia**: tutte le funzioni sono libere; con una donazione si riceve una chiave legata all'**ID macchina** (mostrato nella finestra) che toglie il promemoria giornaliero all'avvio. Chiavi firmate ECDSA P-256: le emette solo l'autore con `tools\LicenseTool` (`keygen` una volta, poi `issue <pem> <ID> "<Nome>"`); la chiave privata sta fuori dal repo (`KaraokeDJ-private\`). Testo in `LICENSE.md`. |
| **Novità 1.3.0** | Cartella download a scelta (Impostazioni); **🧹 Doppioni** (file identici e stesso brano → Cestino); filtro con ON/OFF e risonanza; **VU meter** su master e deck; intro/uscita spostabili con tasto destro sulla forma d'onda (le modifiche manuali non vengono sovrascritte dall'analisi); **❤ Dona** collegato al cloud: pagamento PayPal → chiave istantanea → attivazione automatica. |
